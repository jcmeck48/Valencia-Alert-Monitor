...
# 7. Dead-man's switch
$hb = $cfg.Heartbeat.PingUrl
if (-not $hb) { Add-Result 'Heartbeat' 'SKIP' 'no Heartbeat.PingUrl set - dead-man''s switch is off' }
else {
    try { Invoke-WebRequest -Uri $hb -UseBasicParsing -TimeoutSec 10 | Out-Null; Add-Result 'Heartbeat' 'PASS' 'ping sent - the check should now show green on healthchecks.io' }
    catch { Add-Result 'Heartbeat' 'FAIL' "ping failed: $($_.Exception.Message) - check the URL / internet access" }
}
